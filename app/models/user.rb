class User < ApplicationRecord
  ...
  has_many :active_notifications, class_name: 'notfications', foreign_key: 'visitor_id', dependent: :destroy
  has_many :passive_notifications, class_name: 'notfications', foreign_key: 'visited_id', dependent: :destroy
  ...
  def create_notification_follow!(current_user)
    temp = notfications.where(["visitor_id = ? and visited_id = ? and action = ? ",current_user.id, id, 'follow'])
    if temp.blank?
      notification = current_user.active_notifications.new(
        visited_id: id,
        action: 'follow'
